...
async function getPlayinfo(ext) {
    ...
    const $ = cheerio.load(data)
    const script = $('script:contains(player_data)').text()
    const json = argsify(script.replace('var player_data=', ''))
    if (json.encrypt != 2) {
        $utils.toast(`Not implemented for encrypt is ${json.encrypt}`);
        return
    }
    url = json.url
    const base64Decoded = base64Decode(url)
    const playUrl = decodeURIComponent(base64Decoded)
